 ...
 * Called once per search instead of toolsÃ—termsÃ—2 times.
 */
...
async function searchToolsWithKeywords(
  ...
  // to the full tool set â€” selecting an already-loaded tool is a harmless
  ...
  const scored = await Promise.all(
    candidateTools.map(async tool => {
      ...
      for (const term of allScoringTerms) {
        ...
        // searchHint match â€” curated capability phrase, higher signal than prompt
...
export const ToolSearchTool = buildTool({
  ...
  async call(input, { options: { tools }, getAppState }) {
    ...
    if (selectMatch) {
      ...
      if (found.length === 0) {
        logForDebugging(
          `ToolSearchTool: select failed â€” none found: ${missing.join(', ')}`,
        )
      ...
      if (missing.length > 0) {
        logForDebugging(
          `ToolSearchTool: partial select â€” found: ${found.join(', ')}, missing: ${missing.join(', ')}`,
        )
      } else {
  ...
  mapToolResultToToolResultBlockParam(
    ...
    if (content.matches.length === 0) {
      ...
      if (
        ...
        text += `. Some MCP servers are still connecting: ${content.pending_mcp_servers.join(', ')}. Their tools will become available shortly â€” try searching again.`
      }
      return {
        type: 'tool_result',
